...
def pytest_generate_tests(metafunc):
    if isinstance(metafunc.cls, TestMatrixMixin):
        func_name = metafunc.function.__name__[5:].upper()
        func_data = getattr(metafunc.cls, (func_name + TestMatrixMixin.FIXTURE_SUFFIX), None)
...
def pytest_itemcollected(item):
    if isinstance(item.cls, TestMatrixMixin) and item.name in item.cls.exclude_tests:
